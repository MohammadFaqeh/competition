...
function openCommitteeStartConfirm(participantId){
  ...
  openModal(`<div class="modal-head"><h2>تأكيد بيانات المتسابق قبل البدء</h2><button type="button" class="icon-btn" data-close title="إغلاق"><i data-lucide="x"></i></button></div><div class="modal-body"><p class="field-help">تأكدوا من مطابقة هذه البيانات مع الطالب الحاضر أمامكم قبل بدء تسجيل الأخطاء.</p><p class="field-help">الاسم: <b>${escapeHtml(participant.name)}</b></p><p class="field-help">المركز: <b>${escapeHtml(participant.center||"—")}</b></p><p class="field-help">المستوى: <b>${escapeHtml(participant.levelName||`${participant.level} أجزاء`)}</b></p><p class="field-help">الأجزاء المشارك فيها: <b>${escapeHtml(partsText)}</b></p><div id="committeeConfirmIssueBox" class="hidden"><label>ايش المشكلة بالضبط؟<textarea id="committeeConfirmIssueText" rows="3" placeholder="مثال: الأجزاء المسجّلة غير التي حفظها الطالب فعليًا"></textarea></label><p id="committeeConfirmIssueError" class="form-error hidden"></p></div></div><div class="modal-actions"><button type="button" class="secondary-btn" id="committeeConfirmIssueBtn">إبلاغ عن خطأ</button><button type="button" class="primary-btn" id="committeeConfirmStartBtn">تأكيد والبدء</button></div>`);
