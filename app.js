...
function renderCommitteeStudents(){
  ...
  $("#committeeStudents").innerHTML=eligiblePage.length?eligiblePage.map(participant=>{
    ...
    const action=withdrawn?`<button class="secondary-btn" disabled>منسحب من المسابقة</button>`:status==="manual_dr"?`<button class="secondary-btn" disabled>سُجلت العلامة يدويًا من الإدارة</button>`:canSelfDrawThis?`<button class="primary-btn" data-self-draw="${participant.id}">تسجيل الأجزاء وتنفيذ السحب</button>`:!draw?`<button class="secondary-btn" disabled>بانتظار سحب الإدارة</button>`:status==="pending"?`<button class="primary-btn" data-committee-confirm-start="${participant.id}">البدء بالاختبار الآن</button>`:status==="in_progress"?`<div class="committee-action-group"><button class="primary-btn" data-committee-student="${participant.id}">متابعة الرصد</button>${chairman?`<button type="button" class="compact-btn danger-compact" data-cancel-exam="${participant.id}"><i data-lucide="rotate-ccw"></i> إلغاء الاختبار</button>`:""}</div>`:`<button class="secondary-btn" data-committee-student="${participant.id}">عرض التقييم</button>`;
