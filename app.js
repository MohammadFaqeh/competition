...
function saveState(){localStorage.setItem(activeStorageKey(),JSON.stringify(state));if(operationMode==="cloud"&&cloudEnabled){if(window.CloudCompetition.context?.kind==="subAdmin")window.CloudCompetition.queueSubAdminParticipantsSave(state.participants,error=>toast(`تعذر مزامنة البيانات: ${error.message}`));else window.CloudCompetition.queueStateSave(state,error=>toast(`تعذر مزامنة بيانات الإدارة: ${error.message}`))}}
...
async function setupCloudAdminPanel(){ensureCommitteeMemberFields();renderCommitteeLevelOptions();$("#cloudCommitteesPanel").classList.remove("hidden");$("#subAdminsPanel").classList.remove("hidden");$("#drRequestsPanel").classList.remove("hidden");$("#activityLogPanel").classList.remove("hidden");$("#syncCloudBtn").classList.remove("hidden");renderDrRequests();await Promise.all([renderCloudCommittees(),renderFinalEditAudit(),renderSubAdmins(),renderActivityLog()])}
...
async function renderActivityLog(){
  try{
    const entries=await window.CloudCompetition.listActivityLog(150);
    const labels={create_committee:"إنشاء لجنة",update_committee:"تعديل لجنة",delete_committee:"حذف لجنة",create_sub_admin:"إنشاء حساب مسؤول فرعي",update_sub_admin:"تعديل حساب مسؤول فرعي",delete_sub_admin:"حذف حساب مسؤول فرعي",assign_participant_to_committee:"نقل متسابق للجنة",unassign_participant_from_committee:"إلغاء نقل متسابق",sub_admin_save_participants:"حفظ بيانات متسابقين (مسؤول فرعي)",sub_admin_draw:"سحب موضع (مسؤول فرعي)",admin_draw:"سحب موضع (الإدارة)",committee_draw:"سحب موضع (لجنة)",committee_change_parts:"تعديل أجزاء متسابق (لجنة)",replace_exam_position:"تغيير موضع أثناء الاختبار",grant_final_edit:"منح صلاحية تعديل النتائج",revoke_final_edit:"سحب صلاحية تعديل النتائج",reopen_final_result:"فتح نتيجة معتمدة للتعديل",revise_final_result:"إعادة اعتماد نتيجة معدلة",finalize:"اعتماد نتيجة",save_examiner_draft:"حفظ مسودة تقييم"};
    $("#activityLogList").innerHTML=entries.length?entries.map(entry=>{
      ...
      const who=details.sub_admin_name?`مسؤول فرعي: ${details.sub_admin_name}`:details.committee_name?`لجنة: ${details.committee_name}`:"الإدارة";
    ...
    }).join(""):`<p class="committee-alerts-empty">لا يوجد نشاط مسجل بعد.</p>`;
...
async function enterCloudContext(context){try{operationMode="cloud";stopCommitteeAutoRefresh();
  ...
  if(context.profile.role!=="admin"){setAdminTheme(context.committee?.responsibleGender==="أنثى"?"rose":"green");$("#app").classList.add("hidden");showScreen("committeeApp");await renderCommitteeWorkspace();startCommitteeAutoRefresh();return}
  setAdminTheme("green");const [remote,sessions,committees]=await Promise.all([window.CloudCompetition.loadCompetitionState(),window.CloudCompetition.listSessions(),window.CloudCompetition.listCommittees()]);if(remote.payload?.config){state={...defaultState(),...remote.payload};localStorage.setItem(CLOUD_STORAGE_KEY,JSON.stringify(state))}else if(state.config){await window.CloudCompetition.saveCompetitionState(state)}if(!state.config){showScreen("setupScreen");return}window.CloudCompetition.markAdminKnownIds(state.participants,state.draws);mergeFinalSessionsIntoState(sessions,committees);showApp()}catch(error){toast(`تعذر فتح البيانات المشتركة: ${error.message}`);showScreen("cloudLoginScreen")}}
...
async function refreshAdminChanges(){if(adminRefreshBusy||window.CloudCompetition.context?.kind!=="admin"||!$("#modal")?.classList.contains("hidden"))return;adminRefreshBusy=true;try{const [remote,sessions,committees]=await Promise.all([window.CloudCompetition.loadCompetitionState(),window.CloudCompetition.listSessions(),window.CloudCompetition.listCommittees()]);if(remote.payload?.config){const previous=JSON.stringify({participants:state.participants,draws:state.draws}),next={...defaultState(),...remote.payload};state=next;window.CloudCompetition.markAdminKnownIds(state.participants,state.draws);mergeFinalSessionsIntoState(sessions,committees);localStorage.setItem(CLOUD_STORAGE_KEY,JSON.stringify(state));if(previous!==JSON.stringify({participants:state.participants,draws:state.draws}))renderAll()}}catch(error){console.warn("Admin auto refresh failed",error)}finally{adminRefreshBusy=false}}
...
function describeCommitteeChange(current,previous){if(current&&previous&&Number(current.level)!==Number(previous.level))return `تم تغيير مستوى المتسابق ${current.name}: من ${previous.level} إلى ${current.level} أجزاء`;if(current&&!previous)return `تم إسناد المتسابق إلى مستوى لجنتكم: ${current.name} (${current.level} أجزاء)`;if(!current&&previous)return `تم تغيير مستوى المتسابق ولم يعد ضمن مستوى لجنتكم: ${previous.name} (${previous.level} أجزاء سابقاً)`;return `تم تحديث بيانات المتسابق: ${current?.name||previous?.name}`}
...
function renderCommitteeStudents(){
  ...
  const allEligible=state.participants.filter(participant=>`${participant.name} ${participant.seat} ${participant.center}`.toLowerCase().includes(query));
...
function navigate(view,{historyMode="push",ui=null}={}){if(!$("#"+view+"View"))view="dashboard";localStorage.setItem(currentViewKey(),view);if(ui)restoreListControls(ui);$$(`.view`).forEach(v=>v.classList.toggle("active-view",v.id===`${view}View`));$$(`[data-view]`).forEach(b=>b.classList.toggle("active",b.dataset.view===view));$(".sidebar").classList.remove("open");if(view==="draw"){refreshDrawParticipants();const count=$("#availableCount");if(count&&!integrity.valid)count.textContent="تُجهّز بيانات القرآن عند السحب"}if(view==="participants")renderParticipants();if(view==="history")renderHistory();if(view==="analytics")renderAnalytics();if(view==="settings")populateRenameCenterOptions();if(historyMode!=="none")recordBrowserRoute({surface:"admin",view},{replace:historyMode==="replace"});requestAnimationFrame(()=>window.scrollTo(0,ui?.scrollY||0));lucide.createIcons()}
...
function openParticipantModal(participant=null){
  ...
  $("#participantForm").addEventListener("submit",async event=>{
    ...
    try{
      const item={id:participant?.id||uid("P"),name:$("#pName").value.trim(),seat:$("#pSeat").value.trim(),gender:$("#pGender").value,center:$("#pCenter").value.trim(),branch:$("#pBranch").value.trim()||null,phone:$("#pPhone").value.trim()||null,age:Number($("#pAge").value)||null,level,levelName,parts,createdAt:participant?.createdAt||new Date().toISOString()};
...
function renderParticipants(){
  ...
  $("#participantsTable").innerHTML=list.length?list.map(p=>{const status=statusOf(p),hasDraw=drawByParticipant.has(p.id),passed=Number.isFinite(p.score)&&p.score>=PASS_SCORE,isManualDr=p.scoreSource==="manual"&&Boolean(p.manualEntryBy),hasPendingDr=p.drRequest?.status==="pending";let scoreCell;if(isManualDr){scoreCell=`<div class="dr-score-cell"><b>${p.score}</b><small class="manual-dr-tag">مسجّلة يدويًا${p.manualEntryBy?` · ${escapeHtml(p.manualEntryBy)}`:""}</small>${isMainAdmin?`<button class="compact-btn danger-compact" data-cancel-dr="${p.id}">إلغاء التسجيل اليدوي</button>`:""}</div>`}else if(hasPendingDr){scoreCell=`<span class="score-help">طلب DR بانتظار الموافقة (${formatAssessmentNumber(p.drRequest.score)})</span>`}else if(!hasDraw){scoreCell=`<span class="score-help">تُدخل بعد إجراء السحب</span>`}else if(isSubAdmin){scoreCell=`<button class="compact-btn" data-request-dr="${p.id}"><i data-lucide="file-edit"></i> طلب DR</button>`}else{scoreCell=`<input class="score-input ${Number.isFinite(p.score)?"score-saved":""}" data-score="${p.id}" type="number" min="0" max="100" step="0.01" value="${Number.isFinite(p.score)?p.score:""}" placeholder="أدخل العلامة">`}return `<tr><td><strong>${escapeHtml(p.seat)}</strong></td><td><strong>${escapeHtml(p.name)}</strong></td><td>${escapeHtml(p.gender||"غير محدد")}</td><td>${p.center?escapeHtml(p.center):`<span class="missing-center-tag">⚠ بلا مركز</span>`}</td><td>${escapeHtml(p.levelName||`${p.level} أجزاء`)}</td><td>${status==="completed"?`<span class="state completed">مكتمل</span><span class="outcome ${passed?"pass":"fail"}">${passed?"ناجح":"راسب"}</span>${p.scoreSource==="electronic"?`<small class="electronic-score-tag">تقييم إلكتروني</small>`:""}`:`<span class="state ${status}">${status==="drawn"?"تم السحب · أدخل العلامة":"لم يُسحب بعد"}</span>`}</td><td>${scoreCell}</td><td><div class="row-actions">${hasDraw?`<button class="compact-btn" data-result-participant="${p.id}"><i data-lucide="eye"></i> النتيجة</button>`:`<button class="compact-btn" data-draw="${p.id}"><i data-lucide="sparkles"></i> إجراء السحب</button>`}<details class="dropdown-menu row-actions-more"><summary class="icon-btn" title="المزيد من الإجراءات"><i data-lucide="more-vertical"></i></summary><div class="row-actions-more-list"><button class="compact-btn" data-edit="${p.id}"><i data-lucide="pencil"></i> تعديل</button>${operationMode==="cloud"?`<button class="compact-btn" data-assign-committee="${p.id}"><i data-lucide="shuffle"></i> نقل</button>`:""}<button class="compact-btn danger-compact" data-delete-participant="${p.id}"><i data-lucide="trash-2"></i> حذف</button></div></details></div></td></tr>`}).join(""):`<tr><td class="table-empty" colspan="8">لا توجد أسماء مطابقة</td></tr>`;
...
function currentActorLabel(){const kind=window.CloudCompetition?.context?.kind;if(kind==="subAdmin")return `مسؤول فرعي: ${window.CloudCompetition.context.subAdmin.name}`;if(kind==="admin")return "الإدارة";return state.config?.adminName||"الإدارة"}
...
function openAssignCommitteeModal(participantId){const participant=state.participants.find(p=>p.id===participantId);if(!participant)return;const isSubAdmin=window.CloudCompetition.context?.kind==="subAdmin",committees=isSubAdmin?subAdminCommittees:cloudCommittees;const rows=committees.length?committees.map(c=>{const assigned=(c.extraParticipantIds||c.extra_participant_ids||[]).includes(participantId);return `<label class="committee-member-toggle"><input type="checkbox" data-assign-target="${c.id}" ${assigned?"checked":""}> ${escapeHtml(c.name)}</label>`}).join(""):`<p>لا توجد لجان بعد.</p>`;openModal(`<div class="modal-head"><h2>نقل ${escapeHtml(participant.name)} للجان أخرى</h2><button class="icon-btn" data-close><i data-lucide="x"></i></button></div><div class="modal-body"><p class="field-help">يظهر المتسابق عند اللجان المحدّدة إضافةً لمستواه الأصلي، دون تغيير أي بيانات أخرى.</p><div class="committee-member-fields">${rows}</div></div><div class="modal-actions"><button class="primary-btn" type="button" data-close>تم</button></div>`);$$(`[data-assign-target]`).forEach(input=>input.onchange=async()=>{input.disabled=true;try{await window.CloudCompetition.assignParticipantToCommittee(input.dataset.assignTarget,participantId,input.checked);toast(input.checked?"تم النقل":"تم إلغاء النقل")}catch(error){input.checked=!input.checked;toast(error.message)}finally{input.disabled=false}})}
...
async function importCsv(event){
  ...
  try{
    ...
    let added=0,updated=0,empty=0,invalidLevel=0,awaitingParts=0,excludedSpecial=0,ignoredSheets=0,missingCenter=0;const importedCenters=new Set(),rejectedNames=[],levelKeptNames=[];
    for(const source of sources){
      ...
      for(const row of parsed.rows){
        ...
        const branch=String(pickColumn(row,["الفرع","branch"])||"").trim();
        ...
        if(existing){
          ...
          existing.name=String(name).trim();existing.gender=gender;existing.center=center;existing.phone=phone||null;existing.branch=branch||null;existing.age=age;existing.recitation=recitation||"حفص عن عاصم";
          ...
          state.participants.push({id:uid("P"),name:String(name).trim(),seat:seat||nextSeat(),gender,center,phone:phone||null,branch:branch||null,age,level,levelName,parts,recitation:recitation||"حفص عن عاصم",createdAt:new Date().toISOString()});
...
function assessmentPositionHtml(position,drawPosition,index,total){const positionDeduction=calculateAssessment({positions:[position]}).totalDeduction,types=Object.keys(ASSESSMENT_RULES).filter(type=>type!=="positionChange"||currentExaminerRole()==="chairman");return `<article class="exam-position-card"><div class="exam-position-label"><span>الموضع ${index+1} من ${total}</span><b data-position-deduction="${index}">خصم الموضع: ${formatAssessmentNumber(positionDeduction)}</b></div><h2>${escapeHtml(positionTitle(drawPosition))}</h2><p>الجزء ${drawPosition.juz} · الصفحة ${drawPosition.page}</p>${quranPageHtml(drawPosition)}<div class="exam-actions">${types.map(type=>assessmentActionHtml(position,index,type)).join("")}</div><label class="exam-note">ملاحظات الموضع<textarea data-assess-note="${index}" rows="2" placeholder="ملاحظة اختيارية عن أداء المتسابق">${escapeHtml(position.note||"")}</textarea></label><button type="button" class="secondary-btn exam-position-complete ${position.completed?"is-done":""}" data-toggle-complete="${index}"><i data-lucide="${position.completed?"check-circle-2":"circle"}"></i> ${position.completed?"أُنهي هذا الموضع":"إنهاء هذا الموضع"}</button></article>`}
...
function hydrateSettings(){$("#settingsCompetitionName").value=state.config.competitionName;$("#settingsAdminName").value=state.config.adminName;$("#settingsShowFullQuran").checked=Boolean(state.config.showFullQuranStats);populateRenameCenterOptions()}
...
function closeModal(){document.body.classList.remove("exam-fullscreen");$("#modal").classList.add("hidden");$("#modalContent").innerHTML="";if(!applyingBrowserHistory&&history.state?.marker===HISTORY_MARKER&&history.state.modal){const entry={...history.state};delete entry.modal;history.replaceState(entry,"",location.href)}}
