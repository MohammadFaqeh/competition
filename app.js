...
async function renderMonitorCommittees(){
  try{
    monitorSessions=await window.CloudCompetition.listSessions();
    ...
    renderMonitorDetail();
  }catch(error){console.warn("Monitor refresh failed",error)}
}
...
async function renderActivityLog(){
  try{
    ...
    $("#activityLogList").innerHTML=entries.length?entries.map(entry=>{
      ...
      const who=details.supervisor_name?`مشرف المسابقة: ${details.supervisor_name}`:details.sub_admin_name?`مسؤول فرعي: ${details.sub_admin_name}`:details.committee_name?`لجنة: ${details.committee_name}`:"الإدارة";
...
function openElectronicAssessment(draw,cloudSession=null,jumpToIndex=null){
  ...
  let quranPanelOpen=false,quranPanelIndex=-1;
