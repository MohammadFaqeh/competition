...
async function renderScoreComparison(){
  ...
  try{
    ...
    scoreComparisonRows=sessions.filter(session=>session.assessment&&Object.keys(session.assessment).length).map(session=>{
      ...
      return {name:participant?.name||session.participant_id,committeeName:committee?.name||"—",chairmanScore,memberScore,finalScore};
...
function renderCommitteeStudents(){
  ...
  const statusOrder={in_progress:0,pending:1,no_draw:2,final:3,manual_dr:4,withdrawn:4};
  ...
  $("#committeeCompletedCount").textContent=formatNumber(allEligible.filter(participant=>["final","manual_dr","withdrawn"].includes(statusOf(participant))).length);
  ...
  $("#committeeStudents").innerHTML=eligiblePage.length?eligiblePage.map(participant=>{
    ...
    const statusText=status==="withdrawn"?"منسحب"+(canSeeScore?" · العلامة 0":""):status==="manual_dr"?(canSeeScore?`مسجّلة يدويًا من الإدارة · ${participant.score}`:"مسجّلة يدويًا من الإدارة"):status==="no_draw"?"لم يتم السحب بعد":status==="final"?(canSeeScore?`مكتمل · ${session.score}`:"مكتمل · العلامة غير ظاهرة للجنة"):status==="in_progress"?"مسودة محفوظة":"جاهز للاختبار";
