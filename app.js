...
function openAssignCommitteeModal(participantId){
  ...
  const currentId=participant.transferCommitteeId||null,currentCommittee=currentId?committees.find(c=>c.id===currentId):null;
  const rows=committees.length?committees.map(c=>`<label class="committee-member-toggle"><input type="radio" name="transferTarget" data-transfer-target="${c.id}" ${c.id===currentId?"checked":""}> ${escapeHtml(c.name)}</label>`).join(""):`<p>لا توجد لجان بعد.</p>`;
  openModal(`<div class="modal-head"><h2>نقل ${escapeHtml(participant.name)}</h2><button class="icon-btn" data-close><i data-lucide="x"></i></button></div><div class="modal-body"><p class="field-help">${currentId?`اللجنة الحالية (نُقل يدويًا): <b>${escapeHtml(currentCommittee?.name||"—")}</b>`:"لم يُنقل يدويًا — يظهر حاليًا حسب مستواه الطبيعي."} اختيار لجنة أخرى ينقل المتسابق إليها فورًا ويُخفيه عن لجنته الحالية.</p><div class="committee-member-fields">${rows}</div>${currentId?`<button type="button" id="cancelTransferBtn" class="secondary-btn">إلغاء النقل (إعادة لمستواه الأصلي)</button>`:""}</div><div class="modal-actions"><button class="primary-btn" type="button" data-close>تم</button></div>`);
  ...
  $$(`[data-transfer-target]`).forEach(input=>input.onchange=()=>{const targetId=input.dataset.transferTarget;if(targetId===currentId)return;doTransfer(targetId,`نقل ${participant.name} إلى لجنة ${committees.find(c=>c.id===targetId)?.name||""}؟ سيختفي فورًا من لجنته الحالية.`)});
