...
function bindEvents(){
  ...
  $("#refreshIssueReportsBtn").addEventListener("click",renderIssueReports);
}
...
function populateScoreComparisonCommitteeFilter(){
  ...
  select.innerHTML=`<option value="all">اللجنة: الكل</option>`+scoreComparisonCommittees.map(c=>`<option value="${c.id}">${escapeHtml(committeeLabelWithRoles(c))}</option>`).join("");
  select.value=scoreComparisonCommittees.some(c=>c.id===current)?current:"all";
}
...
async function refreshCommitteeChanges(){if(committeeRefreshBusy||window.CloudCompetition.context?.kind!=="committee"||!$("#modal")?.classList.contains("hidden"))return;committeeRefreshBusy=true;try{await window.CloudCompetition.refreshCommitteeAccess();const committee=window.CloudCompetition.context?.committee;const [remote,sessions]=await Promise.all([window.CloudCompetition.loadCompetitionState(),window.CloudCompetition.listLiveCommitteeSessions(new Date(Date.now()-LIVE_RECENT_WINDOW_MS).toISOString())]);if(!remote.payload?.config||!committee)return;const previousState=state,nextState=committeeScopedState(remote.payload,committee),previousById=new Map(previousState.participants.map(item=>[item.id,item])),nextById=new Map(nextState.participants.map(item=>[item.id,item])),previousDraws=new Map(previousState.draws.filter(draw=>draw.participantId).map(draw=>[draw.participantId,draw])),nextDraws=new Map(nextState.draws.filter(draw=>draw.participantId).map(draw=>[draw.participantId,draw])),changed=nextState.participants.filter(participant=>{const previous=previousById.get(participant.id);return !previous||participantCloudSignature(previous,previousDraws.get(participant.id))!==participantCloudSignature(participant,nextDraws.get(participant.id))}),removed=previousState.participants.filter(previous=>!nextById.has(previous.id));
  ...
  committeeSessions=(()=>{const byId=new Map(committeeSessions.map(item=>[item.id,item]));sessions.forEach(item=>byId.set(item.id,item));return [...byId.values()].filter(item=>nextById.has(item.participant_id))})();
...
function addCommitteeAlerts(items){const now=new Date().toISOString(),alerts=[...items.map((item,index)=>({id:`${Date.now()}-${index}`,text:item.text,participantId:item.participantId||null,at:item.at||now,read:false})),...committeeAlerts()].slice(0,50);localStorage.setItem(committeeAlertsKey(),JSON.stringify(alerts));renderCommitteeAlerts()}
...
function renderCommitteePassRate(committee){
  ...
  // بناءً على طلب صريح (تراجع مؤقت أثناء الاختبار الفعلي): هذه البطاقة تتحكم بها الإدارة الآن
  // عبر نفس زر "إخفاء/إظهار العلامة عن اللجنة" الموجود أصلاً بشاشة إدارة اللجان — بدون أي نشر
  // جديد أو تعديل قاعدة بيانات، الزر شغّال فورًا لكل لجنة على حدة. لو احتجنا لاحقًا فصلها عن
  // إخفاء العلامة الفردية بمفتاح مستقل خاص فيها فقط، هاي نقطة تحتاج عمود DB جديد ومهاجرة SQL —
  // نؤجّلها لوقت هادئ خارج وقت الاختبار الفعلي.
...
function renderCommitteeStudents(){
  ...
  $("#committeeStudents").innerHTML=eligiblePage.length?eligiblePage.map(participant=>{
    ...
    const statusText=withdrawn?"منسحب"+(canSeeScore?" · العلامة 0":""):status==="manual_dr"?(canSeeScore?`مسجّلة يدويًا من الإدارة · ${participant.score}`:"مسجّلة يدويًا من الإدارة"):status==="no_draw"?"لم يتم السحب بعد":status==="final"?(canSeeScore?`مكتمل · ${session.score}`:"مكتمل · العلامة غير ظاهرة للجنة"):status==="in_progress"?"مسودة محفوظة":"جاهز للاختبار";
...
async function cancelCommitteeExam(participantId){
  ...
  try{
    ...
    localStorage.removeItem(ASSESSMENT_DRAFT_PREFIX+participantId);
...
function showResult(draw){
  ...
  if(participant&&canEditParticipantScore(participant))$("#saveResultScore").onclick=()=>{const input=$("#resultScore"),score=Number(input.value);if(input.value===""||!Number.isFinite(score)||score<0||score>100)return toast("أدخل علامة صحيحة بين 0 و100");participant.score=Math.round(score*100)/100;participant.gradedAt=new Date().toISOString();participant.scoreSource="manual";participant.manualEntryBy=currentActorLabel();saveState();renderAll();closeModal();toast("تم حفظ العلامة اليدوية")};
}
...
async function finalizeElectronicAssessment(draw,participant,result){const button=$("#finalizeAssessmentBtn");if(button?.disabled||participant.assessment?.status==="final")return toast("هذه النتيجة معتمدة مسبقاً");if(button){button.disabled=true;button.textContent="جاري اعتماد النتيجة..."}
  ...
  const assessment=participant.assessment,now=new Date().toISOString();assessment.status="final";assessment.finalizedAt=now;assessment.updatedAt=now;assessment.result=result;participant.score=result.score;participant.gradedAt=now;participant.scoreSource="electronic";saveState();
...
function centerOptionsList(){const configured=state.config?.centers||[],fromParticipants=state.participants.map(p=>p.center).filter(Boolean);return [...new Set([...DEFAULT_CENTERS,...configured,...fromParticipants])].sort((a,b)=>String(a).localeCompare(String(b),"ar"))}
