...
-- مع إضافة شرط واحد لفحص المستوى: لجنة إناث + متسابقة غير مسجَّلة ذكراً (أنثى أو بلا جنس — بيانات قديمة) وغير منقولة يدوياً للجنة أخرى (الذكور بلا أي تغيير)،
...
begin
  ...
  if not (p_level=any(v_committee.levels))
     ...
     and not (v_committee.responsible_gender='أنثى' and coalesce(v_gender,'')<>'ذكر' and coalesce(v_transfer_committee_id,'')='') then
