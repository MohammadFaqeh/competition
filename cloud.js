...
window.CloudCompetition=(()=>{
  ...
  async function saveCommittee(values){const {data,error}=await client.rpc("admin_save_committee_v2",{p_id:values.id||null,p_name:values.name,p_chairman_code:values.code,p_chairman_pin:values.pin||"",p_member_code:values.memberCode,p_member_pin:values.memberPin||"",p_levels:values.levels,p_active:values.active!==false});if(error)throw rpcError(error);return data}
  ...
  return {enabled,init,signInAdmin,signInCommittee,refreshCommitteeAccess,signOut,loadCompetitionState,saveCompetitionState,queueStateSave,listCommittees,saveCommittee,setCommitteeActive,deleteCommittee,setCommitteeFinalEdit,listFinalEditAudit,deleteParticipantSession,listSessions,claimStudent,createCommitteeDraw,createAdminDraw,replaceCommitteePosition,saveSession,queueSessionSave,log,get context(){return context},get client(){return client}};
})();
