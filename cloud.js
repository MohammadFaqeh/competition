...
window.CloudCompetition=(()=>{
  ...
  async function loadAdminContext(user){
    const {data:profile,error}=await client.from("profiles").select("id,role,display_name,can_edit_final,can_delete_data,can_transfer_participant").eq("id",user.id).single();
  ...
  async function setSubAdminPermissions(id,canEditFinal,canDeleteData,canTransferParticipant){const {error}=await client.rpc("admin_set_sub_admin_permissions",{p_id:id,p_can_edit_final:Boolean(canEditFinal),p_can_delete_data:Boolean(canDeleteData),p_can_transfer_participant:Boolean(canTransferParticipant)});if(error)throw rpcError(error)}
  ...
  async function linkSupervisor(values){const {data,error}=await client.rpc("admin_link_supervisor",{p_user_id:values.userId,p_name:values.name,p_can_edit_final:Boolean(values.canEditFinal),p_can_delete_data:Boolean(values.canDeleteData),p_can_transfer_participant:Boolean(values.canTransferParticipant)});if(error)throw rpcError(error);return data}
...
// المتسابقين/السحوبات فقط (diwan_state، راجع supabase/diwan-al-hifadh-core.sql). اللجان
// مشتركة تماماً مع السنوية — طلب صريح: "نفس اللجان يلي بضيفهم من الإعدادات يقدروا يمتحنوا
// الطرفين". يستعير نفس اتصال Supabase ونفس جلسة تسجيل دخول اللجنة/الإدارة من CloudCompetition
// (window.CloudCompetition.context.token للجنة) بلا أي حساب أو تسجيل دخول ثانٍ إطلاقاً.
window.DiwanCompetition=(()=>{
  let saveTimer=null,saveGeneration=0;
  ...
  // الإدارية أعلاه (تلك تتطلب صلاحية admin ولا يمكن للجنة استدعاءها بجلسة p_token). مفتاح draw_id
  // لا participant_id — نفس المشارك قد يملك محاولات/جلسات سابقة أخرى يجب ألا تتأثر.
  async function cancelSession(drawId){const {error}=await client().rpc("diwan_committee_cancel_session",{p_token:committeeToken(),p_draw_id:drawId});if(error)throw rpcError(error)}
