...
window.CloudCompetition=(()=>{
  ...
  // "نسيت كلمة السر" لحسابات الإدارة/المشرف — تدفق ذاتي كامل عبر Edge Function (password-reset)
  // تشتغل بصلاحية service_role (لا يوجد المفتاح بالمتصفح إطلاقاً): ترسل رمزاً للإداري الرئيسي
  // فقط، وتحدّث كلمة السر مباشرة على نفس الحساب بعد التحقق من الرمز.
  async function requestPasswordResetCode(email){const {data,error}=await client.functions.invoke("password-reset",{body:{action:"request",email}});if(error)throw rpcError(error);if(data?.ok===false)throw new Error(data.error||"تعذر إرسال الرمز");return data}
  async function confirmPasswordReset(email,code,newPassword){const {data,error}=await client.functions.invoke("password-reset",{body:{action:"reset",email,code,newPassword}});if(error)throw rpcError(error);if(data?.ok===false)throw new Error(data.error||"تعذر تحديث كلمة السر");return data}
  ...
  async function deleteParticipantSession(participantId){if(context?.kind==="supervisor"){const {error}=await client.rpc("supervisor_delete_participant_session",{p_participant_id:participantId});if(error)throw rpcError(error);return}if(context?.kind!=="admin")return;const {error}=await client.rpc("admin_delete_participant_session",{p_participant_id:participantId});if(error)throw rpcError(error)}
